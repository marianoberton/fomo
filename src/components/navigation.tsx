...
export default function Navigation() {
  ...
  return (
    <div className="w-full flex justify-center py-2 md:py-3 fixed top-0 z-50">
      <header 
        className="bg-white/90 backdrop-blur-md text-gray-800 rounded-full shadow-lg border border-gray-200/50 flex items-center justify-between px-4 sm:px-6 h-12 sm:h-14 w-auto max-w-fit transition-all duration-300 hover:shadow-xl"
      >
        <Link href="/" className="flex items-center gap-2 sm:gap-3 group">
          <svg 
            viewBox="0 0 129.44 128.96" 
            className="h-6 w-6 sm:h-7 sm:w-7 group-hover:scale-110 transition-transform duration-300 fill-plum"
          >
          ...
          <span className="text-xl sm:text-2xl font-bold font-concert group-hover:text-brilliantBlue transition-colors duration-300 text-plum">
          FOMO
          </span>
        </Link>
        ...
        <div className="md:hidden flex items-center pl-4">
          <button
            ...
            className="p-2 rounded-full text-gray-700 transition-colors hover:bg-brilliantBlue/10 hover:text-brilliantBlue focus:outline-none"
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <button onClick={() => setIsOpen(false)} className="absolute top-5 right-5 p-2 text-gray-700 hover:bg-brilliantBlue/10 hover:text-brilliantBlue rounded-full" aria-label="Close menu">
              <X size={28} />
            </button>
